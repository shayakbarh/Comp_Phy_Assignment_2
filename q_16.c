...
int main() {
    ...
    for (int i =0; i < n+1 ; i++) {
        ...
        w = w0 + h * f(t0, w0); // Euler's method
        t = t0 + h;

        y = exact_solution(t);
        error = fabs(exact_solution(t) - w); // Error

        error_bound = (h * M * (exp((t - a) * L) - 1))/(2*L); // Error bound

        printf("%.2lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\n", t, exact_solution(t), w, error, error_bound);

        w0 = w; // Update y for the next iteration
        t0 = t;
    }
